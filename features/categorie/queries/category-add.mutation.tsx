...
import { ICategoryAddUpdateResponse } from "../types/categorie.type";
...
export const useAjouterCategoryMutation = () => {
  ...
  return useMutation<ICategoryAddUpdateResponse, Error, CreateCategoryDTO>({
      ...
      if (!validation.success) {
        throw new Error(
          validation.errorsInString ||
            "Erreur lors de la validation des Categories.",
        );
      }

      // Appel de l'action
      const result = await ajouterCategoryAction(
        validation.data as CreateCategoryDTO,
      );
      console.log("Resultat ajout d'une Categorie:", result);
      if (!result.success || !result.data) {
        throw new Error(
          result.error || "Erreur lors de l’ajout  d'une Categorie.",
        );
      }
